...
function updatePermisos(grupo) {
    ...
    fetchCRUD(body, updatePermisosURL).then(data =>{
        ...
        // ⬇️ Invalida cache de permisos del cliente y refresca el gating si existe
        try { sessionStorage.removeItem("perm-cache-v1"); } catch(_) {}
        if (window.PERM_GATE && typeof PERM_GATE.refresh === "function") {
            PERM_GATE.refresh();
        }
    })
}
                ...
                e.addEventListener('click', function () {
                    ...
                    confirmDelete.addEventListener("click",()=>{
                        ...
                        fetchCRUD(body).then(data=>{
                            ...
                            // ⬇️ Invalida cache de permisos y refresca el gating también al eliminar grupo
                            try { sessionStorage.removeItem("perm-cache-v1"); } catch(_) {}
                            if (window.PERM_GATE && typeof PERM_GATE.refresh === "function") {
                                PERM_GATE.refresh();
                            }
