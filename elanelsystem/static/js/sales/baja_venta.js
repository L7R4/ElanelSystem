...
export function openBajaVentaModal() {
  ...
  const btnClose = modal.addFooterBtn(
    "Cancelar",
  ...
  uiMain = createAsyncUI({
    mount: mountMain,
    texts: {
      working: "Procesando baja...",
      success: "Baja realizada",
      ...
      .exec(
        async ({ signal }) => {
          ...
          if (!resp?.status)
            return {
              ok: false,
              message: resp?.message || "Contraseña incorrecta",
            };
    ...
    await uiMain
      .exec(
        async ({ signal }) => {
          const resp = await apiFetch(
            `/ventas/detalle_venta/${ventaId}/dar_baja/`,
            {
              method: "POST",
              data: body,
              signal,
          ...
          if (!resp?.status)
            return {
              ok: false,
              message: resp?.message || "No se pudo realizar la baja",
            };
          return {
            ok: true,
            message: "Baja realizada",
            urlPDF: resp.urlPDF,
            urlUser: resp.urlUser,
          };
        },
        {
          ...
          onSuccessClose: () => {
            ...
            try {
              if (uiMain?.loader && uiMain?.loader.el) {
              }
            } catch {}
      ...
      .then((res) => {
        if (res?.ok) {
          ...
          try {
            if (res.result?.urlPDF) window.open(res.result.urlPDF, "_blank");
          } catch {}
