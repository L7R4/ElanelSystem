...
async function runDelete({ root, signal }) {
  ...
  const resp = await apiFetch(deleteSaleUrl, {
    // `deleteSaleUrl` expuesto globalmente por tu template
    method: "POST",
  ...
  if (!resp || resp.status !== true) {
    return {
      ok: false,
      message: resp?.message || "No se pudo eliminar la venta",
    };
  }
...
export function openModalDeleteVenta() {
  return openActionModal({
    ...
    if (res?.ok && res.result?.urlRedirect) {
      ...
      setTimeout(() => {
        window.location.href = res.result.urlRedirect;
      }, 400);
    }
    return res;
  });
