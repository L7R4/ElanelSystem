 ...
 * Usa helpers de /static/js/vanilla_components/async_ui.js (loader + flujo async).
 */
import {
  createAsyncUI,
  openActionModal,
} from "/static/js/vanilla_components/async_ui.js";
...
const REPORT_URL = (
  $makeInforme?.getAttribute("href") || "/ventas/postventas/informe/"
).split("?")[0];
...
function toggleWrapperDetail(button) {
  const wrapperDetailInfo = button
    .closest(".atributtes")
    .querySelector(".wrapperDetailInfo");
    ...
    <div class="containerHistorialAuditorias">
      ${auditorias
        ...
        <div class="infoCheckWrapper">
          <div class="wrapperComentarios"><p>${
            a.comentarios ?? "Vacio"
          }</p></div>
          <div class="wrapperFechaHora"><p>${a.fecha_hora ?? ""}</p></div>
          <div class="wrapperGrade"><p>${
            a.grade ? "Aprobada" : "Desaprobada"
          }</p></div>
          ${
            i === arr.length - 1
              ? '<div class="wrapperUltimo"><p>Último</p></div>'
              : ""
          }
        </div>
      `
      ...
      <div class="ventaWrapper" id="v${v.id}">
        ...
        <div class="atributtes">
          ...
          <div class="wrapperShortInfo">
            <div class="wrapperInfoAtributte"><h4>Cliente</h4><h1>${
              v.nombre ?? ""
            }</h1></div>
            <div class="wrapperInfoAtributte"><h4>DNI</h4><h1>${
              v.dni ?? ""
            }</h1></div>
            <div class="wrapperInfoAtributte"><h4>Nro Orden</h4><h1>${
              v.nro_operacion ?? ""
            }</h1></div>
            <div class="wrapperInfoAtributte"><h4>Fecha de inscripcion</h4><h1>${
              v.fecha ?? ""
            }</h1></div>
            <div class="wrapperInfoAtributte"><h4>Telefono</h4><h1>${
              v.tel ?? ""
            }</h1></div>
            <div class="wrapperInfoAtributte"><h4>CP</h4><h1>${
              v.cod_postal ?? ""
            }</h1></div>
          </div>

          <div class="wrapperDetailInfo">
            <div class="wrapperInfoAtributte"><h4>Localidad</h4><h1>${
              v.loc ?? ""
            }</h1></div>
            <div class="wrapperInfoAtributte"><h4>Provincia</h4><h1>${
              v.prov ?? ""
            }</h1></div>
            <div class="wrapperInfoAtributte"><h4>Direccion</h4><h1>${
              v.domic ?? ""
            }</h1></div>
            <div class="wrapperInfoAtributte"><h4>Vendedor</h4><h1>${
              v.vendedor ?? ""
            }</h1></div>
            <div class="wrapperInfoAtributte"><h4>Supervisor</h4><h1>${
              v.supervisor ?? ""
            }</h1></div>
            <div class="wrapperInfoAtributte"><h4>Campaña</h4><h1>${
              v.campania ?? ""
            }</h1></div>
            ${audHist}
          </div>
          ...
          <div class="statusWrapper">
            <div class="buttonsWrapper">
              <button id="${btnId}" class="add-button-default" onclick="modalForm('v${
    v.id
  }')">${btnLabel}</button>
            </div>
          </div>
...
function renderCounts(counts) {
  ...
  if ($counts.pendientes)
    $counts.pendientes.textContent =
      counts.pendientes ?? counts.cant_auditorias_pendientes ?? 0;
  if ($counts.realizadas)
    $counts.realizadas.textContent =
      counts.realizadas ?? counts.cant_auditorias_realizadas ?? 0;
  if ($counts.aprobadas)
    $counts.aprobadas.textContent =
      counts.aprobadas ?? counts.cant_auditorias_aprobadas ?? 0;
  if ($counts.desaprobadas)
    $counts.desaprobadas.textContent =
      counts.desaprobadas ?? counts.cant_auditorias_desaprobadas ?? 0;
}
...
async function fetchAuditorias() {
  ...
  const { ok, result } = await ui.exec(
    async ({ signal }) => {
      ...
      if (!res.ok)
        return { ok: false, message: payload?.message || "Error al cargar" };
      return payload;
    },
...
window.modalForm = function modalForm(ventaDomId) {
  const venta_id = String(ventaDomId).startsWith("v")
    ? String(ventaDomId).slice(1)
    : String(ventaDomId);
  ...
  const cls =
    document
      .getElementById("v" + venta_id)
      ?.querySelector(".statusWrapperShortInfo")?.className || "";
  const preset = cls.includes("aprobada")
    ? true
    : cls.includes("desaprobada")
    ? false
    : undefined;
  ...
  openActionModal({
    ...
    validate: (root) => {
      ...
      if (!ok && mount)
        mount.textContent = "Elegí aprobar o desaprobar para habilitar Guardar";
    ...
    run: async ({ signal, root }) => {
      ...
      const payload = await res
        .json()
        .catch(() => ({ status: false, message: "Error de red" }));
      if (!res.ok || payload.status === false) {
        return {
          ok: false,
          message: payload.message || "No se pudo registrar la auditoría",
        };
      }
...
function refreshVenta(ventaUpdated) {
  const ventaElement = document.getElementById(
    `v${ventaUpdated["ventaUpdated_id"]}`
  );
  if (!ventaElement) return;

  const auditorias_updated = Array.isArray(ventaUpdated.auditorias)
    ? ventaUpdated.auditorias
    : [];
  const statusClean_updated =
    ventaUpdated.statusClean || deriveStatusClean(auditorias_updated);
  const statusText_updated =
    ventaUpdated.statusText || deriveStatusText(statusClean_updated);
  ...
  if (!cont) {
    ventaElement
      ...
      ?.insertAdjacentHTML(
        "beforeend",
        `<div class="containerHistorialAuditorias"></div>`
      );
    cont = ventaElement.querySelector(".containerHistorialAuditorias");
  }
  cont.innerHTML = auditorias_updated
    .map(
      (e, i, arr) => `
    <div class="infoCheckWrapper">
  ...
  if (shortInfo) {
    ...
    if (p) p.textContent = statusText_updated;
      ...
      shortInfo.insertAdjacentHTML("beforeend", `<p>${statusText_updated}</p>`);
  }

  // botón -> Editar
  const btns = ventaElement.querySelector(".buttonsWrapper");
  if (btns)
    btns.innerHTML = `<button class="editarButton" onclick="modalForm('v${ventaUpdated["ventaUpdated_id"]}')">Editar</button>`;
}
