...
class FormCreateUser(forms.ModelForm):
    class Meta:
        model = Usuario
        fields = ["nombre", "fec_ingreso","sucursal", "rango" ,"email", "dni", "tel", "domic", "prov", "cp", "loc", "lugar_nacimiento", "fec_nacimiento", "estado_civil", "xp_laboral"]

    # def clean(self):
    #     cleaned_data = super().clean()
    #     self.errors.clear()

    #     # Excluir las validaciones del campo 'password'
    #     try:
    #         self.instance.full_clean(exclude=["password","c"])
    ...
    #     rangos_permitidos = [m.name.lower() for m in Group.objects.all()]

    #     if rango.lower() not in rangos_permitidos:
    #         raise forms.ValidationError('Rango inválido')
    #     return rango
    
    # def clean_sucursal(self):
    #     sucursal = self.cleaned_data['rango']
    #     sucursales_permitidas = [m.pseudonimo.lower() for m in Sucursal.objects.all()]
